  ...
  return (
    <div className="flex flex-col gap-5 animate-fadeUp">
      ...
      <div className="text-center pt-2">
        ...
        <p className="text-muted text-sm mt-2 leading-relaxed max-w-[280px] mx-auto">
          {isChallenger
            ? 'Share the link below. You can both play at the same time!'
      ...
      <div className="bg-surface border border-white/[0.05] rounded-2xl p-4 flex flex-col gap-3">
        <div className="text-xs text-muted/60 font-semibold uppercase tracking-widest text-center">
          {isChallenger ? 'Your challenge link' : 'Challenge ID'}
        </div>
        <div className="flex gap-2">
          <div className="flex-1 bg-surface2 rounded-xl px-3 py-2.5 text-xs text-muted font-mono truncate select-all border border-white/[0.04]">
            {shareUrl}
          </div>
          <button
            onClick={copyLink}
      ...
      {/* How it works */}
      <div className="bg-surface border border-white/[0.05] rounded-2xl p-4 flex flex-col gap-3">
        <div className="text-xs text-muted/60 font-semibold uppercase tracking-widest mb-0.5">How it works</div>
        {[
          ...
          <div key={i} className="flex items-center gap-3 text-sm">
            ...
            <span className="text-muted/80">{step}</span>
          </div>
        ))}
      </div>
