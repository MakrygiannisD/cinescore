...
export default function SessionRevealScreen({
  ...
  // Auto-advance countdown — host drives the transition, non-hosts follow via Realtime
  useEffect(() => {
  ...
  // Realtime: all clients react to the session UPDATE from host's advance_session_round
  const { sendChatMessage } = useSession({
    ...
    onPlayersUpdate: () => {},
    onGuessInsert: () => {
      // Refresh if a late guess arrives
      supabase
        .from('session_guesses')
        .select('*')
        .eq('session_id', session.id)
      ...
      <div className="w-full max-w-md space-y-4">
        ...
        <div className="text-center">
          <p className="text-muted text-xs uppercase tracking-widest mb-1">
            Round {round + 1} of 5 — Actual Rating
          </p>
          <div className="text-5xl font-black text-[#f5c518] mb-1">
